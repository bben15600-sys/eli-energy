...
export function About() {
  return (
    <section id="about" className="border-b border-border/60 py-20 md:py-28">
      <div className="mx-auto grid max-w-6xl grid-cols-1 items-start gap-10 px-4 md:px-6 lg:grid-cols-2 lg:gap-16">
        <div>
          ...
          <ul className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {bullets.map((bullet) => (
              <li key={bullet.title} className="flex gap-3">
                <span className="mt-0.5 inline-flex size-9 shrink-0 items-center justify-center rounded-lg bg-accent text-accent-foreground">
        ...
        <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-accent via-background to-accent/40 lg:aspect-auto lg:h-full lg:min-h-[520px]">
          <div
            aria-hidden
            className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,var(--primary)/12%,transparent_45%)]"
          />
